...
object ReservationDateTimeForm {
  ...
  //フォーマットのバリデーション
  private[form] def checkReservationDateFormat(reservationDateTime: String): Either[ReservationDateTimeFormError, String] =
  ...
  private[form] def checkConvertReservationDateTime(reservationDateTime: String): Either[ReservationDateTimeFormError ,LocalDateTime] =
    Try {
  ...
  //過去の日付じゃない
  private[form] def checkPastDateTime(reservationDateTime: LocalDateTime): Either[ReservationDateTimeFormError ,LocalDateTime] =
